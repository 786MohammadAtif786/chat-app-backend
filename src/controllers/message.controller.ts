...
export const sendMessage = async (
  ...
  try {
    ...
    // Self-message block
    if (senderId === receiverId) {
      res.status(400).json({ success: false, message: "Khud ko message nahi kar sakte" });
      return;
    }
    ...
    // ✅ Sirf receiver ke room mein emit — sender ko nahi
    io.to(`user_${receiverId}`).emit("new-message", {
      message,
      senderId,
      senderName: req.user.name,
    ...
    // ✅ FCM — sirf receiver ko, sender ko BILKUL NAHI
    const receiver = await User.findById(receiverId);
    if (receiver?.fcmToken) {
      try {
        await admin.messaging().send({
          token: receiver.fcmToken,
          notification: {
            title: req.user.name,  // sender ka naam dikhao
            body: text,
          },
          data: {
            senderId: senderId,
        ...
        console.log("[FCM] Notification sent to:", receiver.name);
      } catch (fcmErr) {
